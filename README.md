...
| 路由 | 中文 | 页面 |
| --- | --- | --- |
| `/` | `/zh/` | 首页（01 能力 · 02 产品 · 03 联系） |
| `/products/` | `/zh/products/` | 产品（DesignDance 规格与计划范围） |
| `/contact/` | `/zh/contact/` | 联系（03） |
| `/about/` | `/zh/about/` | 关于（04） |
| `/privacy/` | `/zh/privacy/` | 隐私政策（05） |
| `/terms/` | `/zh/terms/` | 服务条款（06） |
| `/blog/` | `/zh/blog/` | 笔记索引（07） |
...
- **禁止在标题上方加小标签**（kicker/eyebrow）。编号与标题同行同基线。
...
联系邮箱集中在 `src/content/site/contact-email.ts`，改这一行即全站生效。
...
1. **确认联系邮箱** —— 全站使用 `hello@zero-agentic.com`，定义在 `src/content/site/contact-email.ts`。这是站内唯一一个「看起来确定但未经确认」的值：联系页、四条事由分流、以及隐私政策与服务条款的数据请求条款都指向它。若该邮箱不存在，隐私政策向访客做出的法定数据请求承诺会指向一个收不到信的地址。
...
3. **替换所有 TBD** —— 成立年份、注册辖区、注册地址、团队规模、可用性、定价、法务生效日期。
